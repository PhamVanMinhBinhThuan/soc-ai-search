...
import type { ResultTab } from "@/features/search/components/result-tabs";
import {
  buildAiCorrectedAuditQuestion,
  parseAiCorrectedQuestion,
} from "@/shared/lib/audit-question-format";
import { initialScenario } from "@/features/search/lib/mock-data";
import { toUiError } from "@/shared/services/api/api-error-messages";
import { togglePinHistory } from "@/features/investigations/services/history-api";
import { initialMockResponse } from "@/features/search/services/mock-search-api";
import { isMockMode, searchEvents } from "@/features/search/services/search-api";
import type {
...
export function useSearchWorkflow({
  ...
  const changePage = async (page: number) => {
    ...
    try {
      const { runSearchPlan } = await import("@/features/search/services/search-plan-api");
      const paginatedPlan = {

...
import com.soc.ai.search.summary.domain.SummaryLanguageDetector;
import com.soc.ai.search.summary.domain.SummarySource;
import com.soc.ai.search.summary.domain.SummaryTextValidator;
import com.soc.ai.search.summary.infrastructure.elasticsearch.ElasticsearchSummaryQueryService;
import com.soc.ai.search.summary.infrastructure.elasticsearch.SummaryQueryException;
class ResultSummaryServiceTest {
    ...
    void vietnameseQuestionUsesVietnameseFallbackWhenSummaryQueryFails() {
        ...
        assertThat(result.source()).isEqualTo(SummarySource.FALLBACK);
        assertThat(result.summary()).contains("Truy váº¥n Ä‘Ã£ xÃ¡c thá»±c khá»›p 5 sá»± kiá»‡n SOC");
        verify(llmClient, never()).generateSummary(any());
    }

...
class FollowUpSuggestionServiceTest {
    ...
    void vietnameseQuestionIsPassedToPromptBuilder() {
        ...
        when(llmClient.generateFollowUpSuggestions(any(LlmFollowUpSuggestionsRequest.class)))
                .thenReturn(new LlmResponse("""
                        [
                          {"title":"Top IP failed_login","question":"Thá»‘ng kÃª top 5 IP cÃ³ nhiá»u failed_login nháº¥t trong 24h qua"},
                          {"title":"User bá»‹ áº£nh hÆ°á»Ÿng","question":"Group failed_login theo user trong 24h qua"},
                          {"title":"Xu hÆ°á»›ng theo giá»","question":"Hiá»ƒn thá»‹ failed_login theo giá» trong 24h qua"}
                        ]
                        """, "gemini-test", 18));
        var service = service(llmClient);

        var response = service.suggest(request("TÃ¬m failed_login tá»« CN trong 24h qua"));

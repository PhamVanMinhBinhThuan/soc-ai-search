...
Hoàn thiện một bản MVP có thể demo end-to-end, chạy ổn định trên VPS DigitalOcean và gửi mentor kiểm tra sau 14 ngày.
...
CI/CD, VPS DigitalOcean, domain và HTTPS là hạ tầng bàn giao bản demo theo mục tiêu triển khai của đồ án. Đây không phải chức năng khuyến khích của sản phẩm.
...
| Reverse proxy và TLS | Caddy | Reverse proxy public `80/443`, tự cấp và gia hạn HTTPS với Let's Encrypt |
| Đóng gói | Docker Compose | Đúng yêu cầu MVP và phù hợp một VPS |
| CI/CD | GitHub Actions qua SSH | Test/build, SSH vào VPS, `git fetch/reset` và rebuild Docker Compose |
| VPS | DigitalOcean Droplet Ubuntu, cấu hình demo `2 vCPU / 8 GiB RAM / 160 GiB SSD` | Đủ chỗ chạy Elasticsearch và các container MVP trên cùng máy |
| DNS | Name.com DNS | Trỏ bản ghi `A` về public IPv4 của DigitalOcean Droplet |
...
- Mua domain qua Name.com hoặc nhà cung cấp tùy chọn, hoàn tất email xác minh và chuẩn bị DNS record `A`.
...
- Tạo DigitalOcean Droplet Ubuntu, trỏ public IPv4 bằng DNS, cấu hình firewall/UFW chỉ mở:
  - `22/tcp`: chỉ IP cá nhân nếu có thể;
  - `80/tcp`: internet;
  - `443/tcp`: internet.
- Cài Docker Engine và Docker Compose plugin. Caddy chạy bằng Docker Compose profile `proxy`.
...
- Trỏ domain/subdomain về VPS qua Name.com DNS và để Caddy tự cấp HTTPS.
- Cấu hình Caddy:
  - `/` -> frontend;
  - `/api/*` -> backend;
...
## 10. Checklist DigitalOcean, Caddy và domain

- [ ] Domain đã đăng ký và xác minh email.
- [ ] DigitalOcean Droplet Ubuntu đã tạo.
- [ ] Public IPv4 của Droplet đã xác định và ổn định cho DNS demo.
- [ ] UFW hoặc DigitalOcean Firewall chỉ mở `22`, `80`, `443`.
- [ ] SSH key không commit vào repo.
- [ ] `vm.max_map_count=1048576`.
- [ ] Docker và Compose plugin đã cài.
- [ ] Name.com DNS record `A` trỏ domain về public IPv4 của Droplet.
- [ ] Caddy cấp HTTPS certificate tự động thành công.
...
- [ ] Elasticsearch `9200` không public.
- [ ] PostgreSQL `5432` không public.
- [ ] Keycloak internal port không public trực tiếp, chỉ đi qua Caddy HTTPS.
- [ ] Kibana `5601` không public.
...
DigitalOcean Droplet public IPv4 được dùng cho DNS demo hiện tại. Domain `soc-ai-search.app`, `api.soc-ai-search.app` và `auth.soc-ai-search.app` trỏ bằng Name.com DNS A records về public IPv4 của Droplet. HTTPS do Caddy tự cấp và gia hạn bằng Let's Encrypt; không dùng Route 53, Elastic IP, host Nginx hoặc Certbot trong deployment hiện tại.
...
| Domain chưa active | Không lấy được HTTPS | Mua domain ngày 1, kiểm tra DNS A record trỏ về public IPv4 trước khi chạy Caddy |
...
- [Name.com DNS records](https://www.name.com/support/articles/205188698-managing-dns-records)

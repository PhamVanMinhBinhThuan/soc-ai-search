...
22. Nếu tài liệu có Mermaid, giữ diagram đơn giản và renderable. Không thêm diagram quá phức tạp.
23. Sau khi sửa, chạy kiểm tra text nhanh:
...
25. Báo file đã sửa và tóm tắt cấu trúc README/docs mới.
...
Yêu cầu:
1. Kiểm tra trạng thái repository trước khi sửa.
2. Tạo hoặc cập nhật một tài liệu ngắn phục vụ demo/review, ví dụ `plan/day-12-demo-checklist.md` hoặc section trong README nếu hợp lý.
...
5. Demo checklist phải có phần vận hành nhanh trên VPS:
   ...
   - không dùng `down -v`.
6. Review lại README/docs sau khi rewrite:
...
8. Báo checklist PASS/FAIL theo từng nhóm:
   - hardening;
   - README;
   - docs;
   - demo data/account;
   ...
   - domain smoke;
   - secret hygiene.
9. Liệt kê việc còn cần làm ở Day 13:
   - report;
   - slide;
   - screenshot;
   - video dự phòng;
...
git diff --check
git grep -n -E "AIza|sk-[A-Za-z0-9]|ghp_[A-Za-z0-9]|BEGIN (RSA|OPENSSH|PRIVATE) KEY"
rg -n "Ä|Ã|á»|Æ" README.md docs plan/day-12-demo-checklist.md
